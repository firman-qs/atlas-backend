...
pub struct TxQuestionConceptRepository<'a>
{
    db: &'a DatabaseTransaction,
}

impl<'a> TxQuestionConceptRepository<'a>
{
    ...
    pub async fn create(&self, qc: QuestionConceptNew) -> Result<QuestionConcept, RepositoryError>
    {
        let model = qc.into_active_model().insert(self.db).await?;
        Ok(QuestionConcept::from(model))
    }

    pub async fn exists(&self, question_id: Uuid, concept_id: Uuid)
